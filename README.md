<!-- GitHub Profile README — github.com/Dhruva-Aher -->
<!-- LAST_UPDATED -->Last updated: July 25, 2026<!-- /LAST_UPDATED -->

<div align="center">
  <img src="./assets/hero.svg" width="900" alt="Dhruva Aher — Software Engineer" />
</div>

<br><br>

<!-- SECTION 1: ABOUT ME WITH NEON LAPTOP GRAPHIC -->
<div align="center">
  <img src="./assets/about-me.svg" width="900" alt="About Me" />
</div>

<br><br>

<!-- SECTION 2: LANGUAGES AND TOOLS CARD -->
<div align="center">
  <table width="900">
    <tr>
      <td fill="#161B22" style="background-color: #161B22; border: 1px solid #30363D; border-radius: 12px; padding: 20px;">
        <h3 style="margin-top: 0; color: #FFFFFF;">Languages and Tools</h3>
        <br>
        <div align="center">
          <img src="https://skillicons.dev/icons?i=go,python,java,c,javascript,typescript,postgres,redis,mongodb,docker,kubernetes,aws,gcp,terraform,fastapi,react,nextjs,githubactions,git,linux,bash&theme=dark" alt="Languages and Tools" />
        </div>
      </td>
    </tr>
  </table>
</div>

<br><br>

<!-- SECTION 3: FEATURED REPOSITORIES (NATIVE GITHUB CARD STYLE) -->
<div align="center">
  <table width="900">
    <tr>
      <td width="50%" valign="top" style="background-color: #161B22; border: 1px solid #30363D; border-radius: 12px; padding: 18px;">
        <table width="100%">
          <tr>
            <td align="left">
              <a href="https://github.com/Dhruva-Aher/redislite"><b>redislite</b></a>
            </td>
            <td align="right">
              <sub><code>Public</code></sub>
            </td>
          </tr>
        </table>
        <p>A lightweight, Redis-compatible in-memory data store in Go implementing RESP protocol over raw TCP. Achieved <b>75K+ ops/sec</b> with sub-millisecond P95 latency.</p>
        <p>
          <span style="color: #00ADD8;">●</span> <sub>Go</sub> &nbsp;&nbsp;&nbsp;
          <sub>★ 12</sub> &nbsp;&nbsp;&nbsp;
          <sub>⑂ 4</sub>
        </p>
      </td>
      <td width="50%" valign="top" style="background-color: #161B22; border: 1px solid #30363D; border-radius: 12px; padding: 18px;">
        <table width="100%">
          <tr>
            <td align="left">
              <a href="https://github.com/Dhruva-Aher/Aura"><b>Aura</b></a> &nbsp;<sub><a href="https://aurasys.vercel.app">⚡ Live Demo</a></sub>
            </td>
            <td align="right">
              <sub><code>Public</code></sub>
            </td>
          </tr>
        </table>
        <p>Distributed task orchestration platform supporting priority scheduling, crash recovery, and worker coordination across <b>20,000+ queued tasks</b>.</p>
        <p>
          <span style="color: #3178C6;">●</span> <sub>TypeScript</sub> &nbsp;&nbsp;&nbsp;
          <sub>★ 18</sub> &nbsp;&nbsp;&nbsp;
          <sub>⑂ 6</sub>
        </p>
      </td>
    </tr>
    <tr>
      <td width="50%" valign="top" style="background-color: #161B22; border: 1px solid #30363D; border-radius: 12px; padding: 18px;">
        <table width="100%">
          <tr>
            <td align="left">
              <a href="https://github.com/Dhruva-Aher/JusticeQueue"><b>JusticeQueue</b></a> &nbsp;<sub><a href="https://justicequeuelive.vercel.app">⚡ Live Demo</a></sub>
            </td>
            <td align="right">
              <sub><code>Public</code></sub>
            </td>
          </tr>
        </table>
        <p>AI-powered legal case management platform with 13-step NLP/IR pipeline using natural language processing and LLM tool calling on GCP Vertex AI.</p>
        <p>
          <span style="color: #F7DF1E;">●</span> <sub>JavaScript / Next.js</sub> &nbsp;&nbsp;&nbsp;
          <sub>★ 15</sub> &nbsp;&nbsp;&nbsp;
          <sub>⑂ 3</sub>
        </p>
      </td>
      <td width="50%" valign="top" style="background-color: #161B22; border: 1px solid #30363D; border-radius: 12px; padding: 18px;">
        <table width="100%">
          <tr>
            <td align="left">
              <a href="https://github.com/Dhruva-Aher/FlowBoard"><b>FlowBoard</b></a>
            </td>
            <td align="right">
              <sub><code>Public</code></sub>
            </td>
          </tr>
        </table>
        <p>Real-time collaborative project management SaaS platform with isolated workspaces, role-based access control, and WebSockets live editing.</p>
        <p>
          <span style="color: #3178C6;">●</span> <sub>TypeScript / FastAPI</sub> &nbsp;&nbsp;&nbsp;
          <sub>★ 9</sub> &nbsp;&nbsp;&nbsp;
          <sub>⑂ 2</sub>
        </p>
      </td>
    </tr>
    <tr>
      <td width="50%" valign="top" style="background-color: #161B22; border: 1px solid #30363D; border-radius: 12px; padding: 18px;">
        <table width="100%">
          <tr>
            <td align="left">
              <a href="https://github.com/Dhruva-Aher/ReviewAgent"><b>ReviewAgent / PRBeliefs</b></a>
            </td>
            <td align="right">
              <sub><code>Public</code></sub>
            </td>
          </tr>
        </table>
        <p>GitHub Marketplace app performing automated code reviews by dispatching parallel workers via Redis-backed async queues and LLaMA 3.3 70B.</p>
        <p>
          <span style="color: #3572A5;">●</span> <sub>Python</sub> &nbsp;&nbsp;&nbsp;
          <sub>★ 14</sub> &nbsp;&nbsp;&nbsp;
          <sub>⑂ 5</sub>
        </p>
      </td>
      <td width="50%" valign="top" style="background-color: #161B22; border: 1px solid #30363D; border-radius: 12px; padding: 18px;">
        <table width="100%">
          <tr>
            <td align="left">
              <a href="https://github.com/Dhruva-Aher/Disaster-Relief-Response-Gap-Tracker-DRRGT-"><b>FEMA Aid Gap Tracker</b></a>
            </td>
            <td align="right">
              <sub><code>Public</code></sub>
            </td>
          </tr>
        </table>
        <p>ETL data pipeline and analytics dashboard processing <b>810K+ FEMA disaster records</b> across 3,200+ counties. Deployed on AWS with Terraform.</p>
        <p>
          <span style="color: #3572A5;">●</span> <sub>Python / AWS</sub> &nbsp;&nbsp;&nbsp;
          <sub>★ 11</sub> &nbsp;&nbsp;&nbsp;
          <sub>⑂ 3</sub>
        </p>
      </td>
    </tr>
  </table>
</div>

<br><br>

<!-- SECTION 4: ENGINEERING METRICS CARD -->
<div align="center">
  <img src="./assets/metrics.svg" width="900" alt="Engineering Metrics & Scale" />
</div>

<br><br>

<!-- SECTION 5: PROJECT ARCHITECTURE -->
<div align="center">
  <img src="./assets/architecture.svg" width="900" alt="Project Architecture" />
</div>

<br><br>

<!-- SECTION 6: JOURNEY TIMELINE -->
<div align="center">
  <img src="./assets/timeline.svg" width="900" alt="Journey" />
</div>

<br><br>

<!-- SECTION 7: CONTACT SHIELDS -->
<div align="center">
  <a href="https://dhruvaaher.dev"><img src="https://img.shields.io/badge/Portfolio-0D1117?style=for-the-badge&logo=safari&logoColor=4F8CFF" alt="Portfolio" /></a>
  &nbsp;
  <a href="https://linkedin.com/in/dhruva-aher"><img src="https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=4F8CFF" alt="LinkedIn" /></a>
  &nbsp;
  <a href="mailto:dhruvaaher48@gmail.com"><img src="https://img.shields.io/badge/Email-0D1117?style=for-the-badge&logo=gmail&logoColor=4F8CFF" alt="Email" /></a>
  &nbsp;
  <a href="https://github.com/Dhruva-Aher"><img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=4F8CFF" alt="GitHub" /></a>
</div>

<br><br>

<div align="center">
  <sub>Built with precision · Dhruva Aher</sub>
</div>

<br>
